function [matrix,positions,connection,connectible,fc_abs,w,fc]=PartialOverlap_ray_parallel_temp(n,limit,flagtaken,flagsize,cellradius,flagdistance,mplaces,range,group,overlap)
...
% allocate random directions       
        ...
        connect=temp(connect,:); % connectable neurons and their original position indicies
            ...
            overlapconnect=zeros(n,1);
            
            for g=1:group
                overlapconnect=overlapconnect+ [zeros(1:g*n/group) ;overlap*ones(g*n/group+1:n,1)];
            end
            ...
            %else
            connect(1:length(connect_ind),3)= connect(1:length(connect_ind),3)+overlapconnect; % implementing overlap 
